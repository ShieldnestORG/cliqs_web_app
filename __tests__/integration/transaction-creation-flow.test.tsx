...
describe("Transaction Creation Flow: Complete Navigation Test", () => {
  ...
  it("should create transaction and call router.push with correct URL", async () => {
    ...
    const txId = await createDbTx(mockAccountOnChain.address, mockChain.chainId, {
    ...
    expect(calledPath).toMatch(
      /^\/tx\/core14rmczf6t6qldyrqrv4jd0zzypkuymrhvsxazxf\/transaction\/\d+-[a-z0-9]+$/,
    );
  });
  ...
  it("should navigate to transaction detail page after successful creation", async () => {
    ...
    expect(mockRouterPush).toHaveBeenCalledWith(expect.stringContaining("/transaction/"));
