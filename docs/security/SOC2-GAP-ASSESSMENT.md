...
**Not reviewed — an explicit gap in this assessment's coverage, not a clean bill of health:** the *page* routes under `pages/` (everything except `pages/api/`) were outside the original route sweep, which inventoried API handlers only. Appendix A is therefore an API inventory, not a full attack-surface inventory. One page-route finding surfaced after the fact and is recorded below (`/[chainName]/dev`); the remaining page routes have **not** been assessed, and a future pass should sweep them for the same authorization questions applied to the API.
...
| The `/[chainName]/dev` **page** route renders DevTools unconditionally in production — no `NODE_ENV` check, no `getServerSideProps`, no gate of any kind (VERIFIED: all 47 lines read; `<DevTools />` at `:42`). `showDevTools` (`lib/featureFlags.ts:20-21`) gates only the **nav entry**, and the code comment at `:14-16` states the route still resolves so links and bookmarks keep working — so this is **deliberate, with a stated rationale**, not an oversight. Exposure is *phishing amplification*, not anonymous theft: DevTools builds transactions the victim still signs in their own wallet, but an attacker can hand someone a contract-migration, Update-Admin, or authz-grant signing flow hosted on the genuine `app.cliqs.io` origin (`components/DevTools/DevToolsAuthz.tsx`), defeating the main heuristic users have for spotting fake signing prompts | `pages/[chainName]/dev.tsx`, `lib/featureFlags.ts:14-21` | **Open — decision pending.** Intentional; surfaced 2026-08-17 by the parallel security workstream and referred to the owner as a product decision rather than changed unilaterally. If it is gated, the `featureFlags.ts:14-16` rationale comment becomes false and must be rewritten |
